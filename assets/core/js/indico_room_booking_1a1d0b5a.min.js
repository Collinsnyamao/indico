(function(root){var serverSide=typeof module!=='undefined'&&module.exports;var getnlp=function(){if(!getnlp._nlp){if(serverSide){getnlp._nlp=require('./nlp')}else if(!(getnlp._nlp=root._RRuleNLP)){throw new Error('You need to include rrule/nlp.js for fromText/toText to work.')}}
...
var tt=this.data.getDayTooltip(day);if(tt){tt=tt.replace(/\n/g,'<br>');}
...
if(data.success){var userId=$('body').data('userId');$.jStorage.set('rb-user-{0}'.format(userId),{});location.href=data.url;}else{var error_box=$('.js-booking-creation-error-box').clone().show();var errors=error_box.find('.js-booking-creation-error-message');$.each(data.msg.split('\n'),function addError(_,error){errors.append($('<li>',{text:error}));});new AlertPopup($T("Booking creation error"),error_box[0]).open();}},complete:function(){killProgress();}});});$('.js-confirm-warning').on('change',function(){validate_conflict();});validate_conflict();});})();